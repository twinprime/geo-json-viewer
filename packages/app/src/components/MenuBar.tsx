...
interface MenuBarProps {
  ...
  onDataAppend: (data: GeoJSON) => void
}
...
export const MenuBar: React.FC<MenuBarProps> = ({
  ...
  const handleAppendChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const file = event.target.files?.[0]
    if (file) {
      const reader = new FileReader()
      reader.onload = (e) => {
    ...
    <div className="h-12 bg-gray-900 border-b border-gray-700 flex items-center px-4 shrink-0">
      <div className="flex items-center gap-4">
        ...
        <button
          ...
          className="flex items-center gap-2 px-3 py-1.5 text-sm md:text-base bg-green-600 hover:bg-green-700 text-white rounded transition-colors"
        >
          <FilePlus size={16} />
          Append File
        </button>
